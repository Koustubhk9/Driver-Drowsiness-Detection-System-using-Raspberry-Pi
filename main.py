...
rightEyeHull = cv2.convexHull(rightEye) cv2.drawContours(frame, [leftEyeHull], -1, (0, 255, 0), 1) cv2.drawContours(frame, [rightEyeHull], -1, (0, 255, 0), 1) if ear < thresh:
flag += 1
print (flag)
if flag >= frame_check:
...
flag = 0
cv2.imshow("Frame", frame) key = cv2.waitKey(1) & 0xFF if key == ord("q"):
